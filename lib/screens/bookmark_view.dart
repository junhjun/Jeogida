...
class _BookMarkViewState extends State<BookMarkView> {
  ...
  Future<void> postDataSpot(locationID, userCode) async {
    final url = Uri.parse(haniumURL + "/favorite-location/" + userCode + "/" + locationID);
    final headers = {"Content-type": "application/json"};
    final response = await http.post(url,
        headers: headers,
        body: jsonEncode({
  ...
  Future<void> getData(userCode) async {
    ...
    if ((result_car.statusCode == 200) & (result_spot.statusCode == 200) & (result_everyspot.statusCode == 200)) {
      setState(() {
                            ...
                            child: TextButton(
                              ...
                              onPressed: () {
                                showDialog(
                                    context: context,
                                    builder: (context) {
                                      return Dialogdel(delCar: delCar, delSpot: delSpot, Code: Code);
                                    });
                              },
                              child: Container(
                      ...
                      child: TextButton(
                          ...
                          onPressed: () {
                            showDialog(
                                ...
                                builder: (context) {
                                  return DialogaddCar(
                                      addCarNum: addCarNum, addCheck1: addCheck1, Code: Code, data_car:data_car);
                                });
                          },
                          child: Container(
                            ...
                            showDialog(
                                ...
                                builder: (context) {
                                  return DialogaddSpot(
                                      addSpot: addSpot, addCheck2: addCheck2, Code: Code, data_spot: data_spot, data_everyspot: data_everyspot);
                                });
                          },
                          child: Container(
                            height: MediaQuery.of(context).size.width * 0.5,
...
class DialogaddCar extends StatefulWidget {
  DialogaddCar({Key? key, this.addCarNum, this.addCheck1, this.Code, this.data_car}) : super(key: key);
  final addCarNum;
  final addCheck1;
  final Code;
  final data_car;
            ...
            child: TextButton(
                ...
                onPressed: () {
                  if ((inputData1.text != '') & (inputData2.text != '')) { // 자동차 이름, 차량 번호 모두 입력해야 추가됨
                    widget.addCarNum(inputData1.text, inputData2.text, widget.Code);
                    widget.addCheck1();
                  }
...
class DialogaddSpot extends StatefulWidget {
  DialogaddSpot({Key? key, this.addSpot, this.addCheck2, this.Code, this.data_spot, this.data_everyspot}) : super(key: key);
  final addSpot;
  final addCheck2;
  final Code;
  final data_spot;
        ...
        Container(
          ...
          child: SizedBox(
            ...
            child: TextButton(
                ...
                onPressed: () {
                  if ((selectedValue != '') && !(bmSpotNameList.contains(selectedValue))) { // 즐겨찾기 주차장과 중복되지 않으면
                    for (int i = 0; i < widget.data_everyspot.length; i++) {
